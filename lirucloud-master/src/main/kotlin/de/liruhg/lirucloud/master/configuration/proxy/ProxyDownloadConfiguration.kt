...
class ProxyDownloadConfiguration(
    private val fileHandler: SyncFileHandler
) : Configuration {

    private val logger: Logger = LoggerFactory.getLogger(ProxyDownloadConfiguration::class.java)
    ...
    override fun execute() {
        ...
        try {
            ...
            FileUtils.zipMultipleFiles(File(Directories.MASTER_SOFTWARE_PROXY, "proxy.zip"), setOf(proxyJar))

            this.logger.info("Uploading proxy jar to database bucket...")
            this.fileHandler.uploadFile(
                File(Directories.MASTER_SOFTWARE_PROXY, "proxy.zip"),
