...
class ProxyProcessRequestHandler(
    ...
    override fun handle(process: CloudProcess): Pair<String, Boolean> {
        ...
        this.fileHandler.downloadFile(
            hashedName,
            File(Directories.CLIENT_CACHED_TEMPLATES_PROXY, "${process.groupName}.zip")
        )

        FileUtils.unzipFile(
            File(Directories.CLIENT_CACHED_TEMPLATES_PROXY, "${process.groupName}.zip"),
            "${Directories.CLIENT_CACHED_TEMPLATES_PROXY}/${process.groupName}"
        )

        val serverDirectory =
            File("${Directories.CLIENT_RUNNING_PROCESSES_PROXY}/${process.groupName}_${process.uuid}")

        FileUtils.copyAllFiles(
            File("${Directories.CLIENT_CACHED_TEMPLATES_PROXY}/${process.groupName}/default").toPath(),
        ...
        val message =
            "Successfully started process with Name: [${internalProxyProcess.name}] - UUID: [${internalProxyProcess.uuid}]"
