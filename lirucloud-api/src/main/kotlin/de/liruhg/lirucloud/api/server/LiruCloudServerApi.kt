    ...
    override fun onEnable() {
        ...
        KODEIN.direct.instance<ConfigurationExecutor>().executeConfigurations(
            DefaultFolderCreator(
                setOf(
                    Path.of(Directories.SERVER_PLUGINS),
                    Path.of(Directories.SERVER_PLUGINS_API),
                )
            ),
            DefaultPluginConfiguration(
                File(Directories.SERVER_PLUGINS_API, "config.json"),
        ...
        KODEIN = DI {
            ...
            bindSingleton {
                ...
                configurationExecutor.registerConfiguration(
                    KeyReader(
                        File("${Directories.SERVER_PLUGINS_API}/client.key"),
                        instance()
                    )
                )
